  ...
  return (
    <>
      ...
      <section className="relative text-white py-28 md:py-36 overflow-hidden">
        <Image src={IMAGES.hero.contact} alt="Contact WPTC" fill className="object-cover" priority quality={85} />
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 max-w-6xl mx-auto">
            ...
            <SlideIn direction="left" className="lg:col-span-2 space-y-6">
              <div>
                <h2 className="font-[family-name:var(--font-playfair)] text-2xl font-bold text-charcoal mb-2">
                  Contact Information
                </h2>
                <p className="text-neutral-500">Reach us by phone, email, or visit us in person.</p>
              </div>

              <div className="space-y-5">
                {CONTACT_INFO.map((item) => (
                  <div key={item.label} className="flex items-start gap-4">
                    <div className="shrink-0 w-10 h-10 rounded-xl bg-gold/8 border border-gold/10 flex items-center justify-center text-gold">
                      <item.icon className="h-4 w-4" />
                    </div>
                    <div>
                      <p className="text-[11px] text-neutral-400 uppercase tracking-[0.15em] font-semibold mb-0.5">{item.label}</p>
                      {item.href ? (
                        <a href={item.href} className="text-sm text-charcoal hover:text-gold transition-colors">{item.value}</a>
                      ) : (
              ...
              <div className="mt-8 relative h-[250px] rounded-xl overflow-hidden shadow-lg">
                <Image src={IMAGES.hero.home} alt="Warriors Prayer Tower Complex" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 40vw" />
                <div className="absolute inset-0 bg-gradient-to-t from-charcoal/70 to-transparent" />
                <div className="absolute bottom-4 left-4">
                  <p className="text-sm font-semibold text-white">{SITE.address}</p>
                  <p className="text-xs text-gold/70">Ghana, West Africa</p>
                </div>
              </div>
            </SlideIn>
            ...
            <SlideIn direction="right" delay={0.15} className="lg:col-span-3">
              <Card className="border-0 ring-0 shadow-lg overflow-hidden">
                ...
                <CardContent className="p-6 md:p-8">
                  <h2 className="font-[family-name:var(--font-playfair)] text-xl font-bold text-charcoal mb-6">
                    Send Us a Message
                  </h2>
                  ...
                  <form className="space-y-5" onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input id="firstName" placeholder="Your first name" className="border-neutral-200 focus-visible:ring-gold" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="lastName">Last Name</Label>
                        <Input id="lastName" placeholder="Your last name" className="border-neutral-200 focus-visible:ring-gold" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input id="email" type="email" placeholder="your@email.com" className="border-neutral-200 focus-visible:ring-gold" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="phone">Phone</Label>
                        <Input id="phone" type="tel" placeholder="+233 XXX XXX XXX" className="border-neutral-200 focus-visible:ring-gold" />
                      </div>
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="subject">Subject</Label>
                      <Input id="subject" placeholder="What is this about?" className="border-neutral-200 focus-visible:ring-gold" />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="message">Message</Label>
                      <Textarea id="message" placeholder="Tell us about your retreat plans, group size, preferred dates, or any questions..." rows={5} className="border-neutral-200 focus-visible:ring-gold resize-none" />
                    </div>
                    ...
                    <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={{ type: "spring", stiffness: 200, damping: 25 }}>
                      <Button type="submit" disabled={sending} className="w-full bg-charcoal hover:bg-charcoal-light text-white gap-2 h-11">
                        {sending ? (
